      ...
      <ScrollView className='history' scrollY>
        ...
        {this.state.editing ?
          <View className='historyItem'> 
            <Input className='itemInput' placeholder={this.props.title} onInput={e => this.setState({title: e.detail.value})} focus></Input>
            <View className='itemEdit right' onClick={() => store.updateTodo(this.props.id,this.state.title)}>完成</View>
          </View>
          :
          <View className='historyItem'>
            {/* 删除 */}
            <View className='itemDelete right' onClick={() => this.deleteT(this.props.id)}>删除</View>
            <View className='itemEdit right' onClick={() => this.isEditing(this.props.id)}>编辑</View>
